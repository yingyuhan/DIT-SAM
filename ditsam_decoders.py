...
from models.module_lib import MLP, LayerNorm2d
from models.segment_anything_ext.modeling import TwoWayTransformer, MaskDecoder


class DITDecoder(MaskDecoder):
    ...
    def forward(
        ...
        参数:
        image_embeddings (torch.Tensor): 来自 ViT 图像编码器的嵌入
        ...
        for i_batch in range(batch_size):
            masks, iou_preds = self.predict_masks(
                image_embeddings=image_embeddings[i_batch].unsqueeze(0),        # torch.Size([1, 256, 64, 64])
                image_pe=image_pe[i_batch],      # torch.Size([1, 256, 64, 64])
                sparse_prompt_embeddings=sparse_prompt_embeddings[i_batch],     # tensor([], device='cuda:0', size=(1, 0, 256))
                dense_prompt_embeddings=dense_prompt_embeddings[i_batch],       # torch.Size([1, 256, 64, 64])
                hq_feature=hq_features[i_batch].unsqueeze(0),                   # torch.Size([1, 32, 256, 256])
                hq_token_weight=hq_token_weight                                 # None
            )
            ...
            masks_hq = masks[:, slice(self.num_mask_tokens - 1, self.num_mask_tokens), :, :]  # slice(4, 5)
    ...
    def predict_masks(
        ...
        if hq_token_weight is None:  # None
        ...
        tokens = torch.cat((output_tokens, sparse_prompt_embeddings), dim=1) #稀疏提示插入在这里！

        src = torch.repeat_interleave(image_embeddings, tokens.shape[0], dim=0) 
        src = src + dense_prompt_embeddings # 密集提示插入在这里
        ...
        hyper_in = torch.stack(hyper_in_list, dim=1)  # torch.Size([1, 5, 32])
