...
from models.encoders import SAMImageEncodeWrapper, SAMPromptEncodeWrapper, DITSAMTImageEncoder
from models.decoders import DIT_Decoder
from models.segment_anything_ext import sam_model_registry

from models.forward_info_flow import FeatureFusion
...
class BaseDITSAM(nn.Module):

    def __init__(self, model_type: str):
        super(BaseDITSAM, self).__init__()
        ...
        self.mask_decoder = DIT_Decoder(
            model_type, self.ori_sam.mask_decoder.state_dict()
        )  
        self.fusion_module = FeatureFusion().to('cuda')
    ...
    def train(self, mode: bool = True):
        if mode: 
            for n, c in self.named_children():
                if n not in ['image_encoder', 'fusion_module']:
                    c.eval()
                else:
                    c.train()
        else:
    ...
    def forward(
            ...
            two_way_information_flow: bool = True  
    ):
        ...
        interm_embeddings_torch = torch.stack(interm_embeddings)
        interm_embeddings_torch = torch.transpose(interm_embeddings_torch, 0, 1)
        dense_embeddings_list = []
        for batch_idx in range(batch_size):
            fused_feature = self.fusion_module(image_embeddings[batch_idx], interm_embeddings_torch[batch_idx],two_way_information_flow)
            dense_embeddings_list.append(fused_feature)
    ...
    def preprocess(self,
        ...
        imgs_return, point_coords_return, box_coords_return = \
            copy.deepcopy(imgs), copy.deepcopy(point_coords), copy.deepcopy(box_coords)   贝
...
class DITSAM(BaseDITSAM):
    def __init__(self, model_type: str):
        super(CATSAMT, self).__init__(model_type=model_type)
        self.image_encoder = DITSAMTImageEncoder(ori_sam=self.ori_sam, hq_token=self.mask_decoder.hf_token.weight)
